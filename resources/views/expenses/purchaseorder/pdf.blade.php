    ...
    <table class="table table-bordered mt-5" style="font-size: 16px">
        ...
        <?php $i++?>
          <tr>
            ...
            <td class="number">{{number_format($purchaseorder->saleprice,2)}}</td>
            <td class="number">{{number_format($purchaseorder->saleprice * $purchaseorder->amount,2)}}</td>
          </tr>
          @endforeach
          @foreach ($sums as $sum)
          <tr>
            <td rowspan="3" colspan="3">หมายเหตุ : </td><td>VATABLE</td><td class="number">{{number_format($sum->sum - ($sum->sum * 7/100),2)}}</td>
            </tr>
            <tr>
