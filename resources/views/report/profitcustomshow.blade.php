  ...
  function drawChart() {
    var data = google.visualization.arrayToDataTable([
     ...
     @foreach($incomeline as $line)
      ...
      ['{{$line->Year}} {{$line->Month}}',{{$line->sum}},null],
     @endforeach

     @foreach($expensesline as $lineex)
      ['{{$lineex->Year}} {{$lineex->Month}}',null,{{$lineex->sum}}],
     @endforeach
    ...
    <div class="container mt-5 shadow p-3 mb-5 bg-white rounded">
        ...
        <p style="text-align: center;font-size: 18px;">{{date('d-m-Y', strtotime($fromDate))}} ถึง {{date('d-m-Y', strtotime($toDate))}}</p>
